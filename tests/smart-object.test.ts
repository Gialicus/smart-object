...
type ProfileInstance = SmartObjectInstance<typeof profileSchema>;

const profileWithAgeInitial = { name: "Mario", age: 30 };
...
describe("SmartObject generic union root", () => {
  ...
  it("throws when union root options are not all z.object", () => {
    expect(() => SmartObject(z.union([z.string(), z.number()]))).toThrow(TypeError);
  });
});
